...
int gfx_get_event(
    SDL_Event *event); /* get event (keyboard/mouse/window events). return 0 if there's no event, otherwise return 1 */
...
void gfx_get_text_size(const char *text, int *width,
                       int *height); /* get the width and height of text to be printed on screen */
...
void gfx_rectangle(int x1, int y1, int x2,
                   int y2); /* draw a rectangle of top-left corner (x1,y1) and bottom-right corner (x2,y2) */
